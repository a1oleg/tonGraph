...
Шаг 4  ✅  vector-guided fuzzing: state-vector counters + value profile pairs
```
...
Полный прогон не имеет смысла — переходим к Шагу 4 (vector-guided fuzzing)
если хотим продолжать, либо считаем Phase 2 завершённой.

---

## Шаг 4 — vector-guided fuzzing ✅

**Что добавляет:** семантические state-vector counters поверх code coverage.
Даёт libFuzzer сигнал о *комбинациях состояний* (не только о ветках кода).

**Реализация:** `__sanitizer_cov_trace_cmp1` value-profile pairs — без Faiss/hnswlib,
без изменений production-кода, без PC-table mismatch.

**Запуск:** добавить `-use_value_profile=1` к аргументам fuzzer.

### Что отслеживается (per-slot, 16 слотов × 8 событий)

| Событие | Значение |
|---|---|
| `SE_NOTAR_VOTE` | notarize vote broadcast для этого слота |
| `SE_SKIP_VOTE` | skip vote broadcast для этого слота |
| `SE_FINAL_VOTE` | finalize vote broadcast для этого слота |
| `SE_NOTAR_CERT` | NotarCert наблюдён для этого слота |
| `SE_BOTH_NS` | DANGER: оба notar и skip vote на одном слоте |
| `SE_CERT_SKIP` | DANGER: NotarCert + skip votes на одном слоте |
| `SE_POST_CRASH` | событие на слоте после crash+restart |

Danger-сигналы эмитируют value-profile пары:
- `cmp1(notar_vote_count, skip_vote_count)` — корреляция vote-типов per-slot
- `cmp1(slot | 0x80, skip_vote_count)` — NotarCert + skip накопление
- `cmp1(slot | 0xC0, event)` — post-crash активность на "нотаризованном" слоте

### Результат

| Параметр | Без Step 4 | С Step 4 (-use_value_profile=1) |
|---|---|---|
| `cov:` | 795 | 776 |
| `ft:` (feature targets) | ~1050 | **1950** |

`ft` вырос на ~900 — libFuzzer видит новые семантические пути в тех же итерациях.

### Ограничение: alarm-skip-after-notarize

AlarmSkip требует `Consensus` актора (consensus.cpp) и его зависимостей
(`CandidateResolver`, `StateResolver` со сложным `ChainState` mock).
Регистрация `Consensus::register_in` возможна, но требует stub-резолверов —
отдельная задача (Шаг 4b).
