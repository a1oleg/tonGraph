...
**Что отслеживается:** per-slot (16 слотов × 8 событий) — `SE_NOTAR_VOTE`,
`SE_SKIP_VOTE`, `SE_FINAL_VOTE`, `SE_NOTAR_CERT`, `SE_BOTH_NS` (danger),
`SE_CERT_SKIP` (danger), `SE_POST_CRASH`.

**Результат:** `ft` (feature targets) вырос ~1050 → **1950** (+900 семантических путей).

**Следующий шаг:** полная VectorDB guidance (Faiss/hnswlib) + Consensus актор →
см. [FUZZING_PHASE3.md](FUZZING_PHASE3.md).
