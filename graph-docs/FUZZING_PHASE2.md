...
На что смотреть:
- `crashes_pool/` — любой crash важен (`__builtin_trap` = dual-cert safety violation)
- `cov:` — должна расти дольше (crash paths сложнее)
- Corpus будет расти медленнее чем Step 2 (crash restart дорогой)
