...
> **Примечание:** libFuzzer не умеет автоматически останавливаться при plateau. В следующих прогонах добавить в `fuzz_watch.sh` детектор — мониторить `ft:` в логе, останавливать если не растёт >1 часа:
> ```bash
> ft=$(grep -o 'ft: [0-9]*' fuzz-0.log | tail -1 | awk '{print $2}')
> if [ $stale -gt 3600 ]; then pkill -f fuzz_harness; fi
> ```
