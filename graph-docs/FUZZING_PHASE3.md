...
Phase 2 (Шаги 1–4) завершена: code coverage исчерпан (`cov: 795`), WAL crash injection работает,
state-vector counters добавляют семантический сигнал (`ft: 1950`). Крашей нет.

Это означает: **простые баги в реализации отсутствуют**. Остаются сценарии,
которые требуют точного управления последовательностью состояний — libFuzzer
с random mutation их не находит за разумное время.

Phase 3 = **directed search**: не случайный поиск, а целевое давление на
конкретные invariant-нарушения.
...
```
Шаг 1  🔲  Consensus актор + stub-резолверы (alarm-skip-after-notarize)
Шаг 2  🔲  VectorDB guidance (Faiss/hnswlib)
Шаг 3  🔲  Распределённый запуск (corpus sync + стратегии)
```

---

## Шаг 1 — Consensus актор + stub-резолверы 🔲
...
## Шаг 2 — VectorDB guidance 🔲

### Концепция

Phase 2 Step 4 добавил `__sanitizer_cov_trace_cmp1` value-profile пары —
это упрощённый вариант. Полная vector guidance:
...
### Отличие от value-profile (Phase 2 Step 4)

| | Phase 2 Step 4 | Phase 3 Step 2 |
|---|---|---|
...
- Реконструировать из events (`OutgoingProtocolMessage` + cert events) —
  это то, что уже делает Phase 2 Step 4 через `g_state_counters`

**Оценка:** 1 неделя.

---

## Шаг 3 — Распределённый запуск 🔲
...
Применять если Шаг 2 (VectorDB) не даёт новых крашей за 72 часа.
...
| alarm-skip-after-notarize | Consensus актор + crash | 🔲 Шаг 1 |
| amnesia-gap | crash_losing(ourVote) + bootstrap replay | ✅ частично (Phase 2 Step 3) |
...
```
Phase 2 завершена → code coverage исчерпан
        ↓
Phase 3 Шаг 1: добавить Consensus актор → alarm-skip путь открыт
        ↓ (если плато)
Phase 3 Шаг 2: VectorDB guidance → gradient descent к опасным состояниям
        ↓ (при наличии нескольких машин, параллельно)
Phase 3 Шаг 3: corpus sync + специализация стратегий
        ↓ (если 72ч без крашей)
